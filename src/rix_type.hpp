...
// Trivial types carry no data -- their equality is always true, since any
// two Bools (say) are the same type. Kept as distinct structs rather than
// one shared "EmptyType" so std::variant / std::get<T> can tell them apart.
...
struct ResultType {
    ...
    bool operator==(const ResultType& other) const;   // defined in .cpp: needs RixType complete
...
// RixType is every type a Rix value can have.

class RixType {
public:
    using Variant = std::variant<
        MatrixType, ScalarType, BoolType, IntType, NatType,
        StringType, VoidType, ResultType, StructType
    >;

    explicit RixType(Variant v) : value_(std::move(v)) {}

    RixType() : value_(VoidType{}) {}
